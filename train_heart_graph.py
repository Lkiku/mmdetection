...
def parse_args():
    ...
    parser.add_argument('--suffix', type=str, default='graph', help='suffix to add to the experiment name')
...
def main():
    ...
    # 修改所有需要设置类别数量的地方
    cfg.model.roi_head = dict(
        type='GraphEnhancedRoIHead',
        graph_embedding_dim=256,
        graph_hidden_dim=256,
        enable_graph_aug=True,
    ...
    # 如果使用图增强，需要更新 bbox_head 的输入通道数
    if cfg.model.roi_head.type == 'GraphEnhancedRoIHead':
        cfg.model.roi_head.bbox_head.update({
            'in_channels': 256 + 256,  # 原始特征通道数(256) + 图特征维度(256)
            'roi_feat_size': 7,
            'fc_out_channels': 1024,
        })
