...
def main():
    ...
    # 在LoadAnnotations后添加LocationScaleAugmentation
    if isinstance(cfg.train_pipeline, list):
        print("add my augmentaion--------------------------------------------------------")
        cfg.train_pipeline.insert(2, dict(
            type='LocationScaleAugmentation',
    ...
    cfg.visualizer = dict(
        ...
        vis_backends=[
            dict(type='LocalVisBackend', save_dir='vis_data'),  
            dict(
    ...
    cfg.default_hooks.update({
        'visualization': dict(
            type='CustomDetVisualizationHook',
            draw=True,
            interval=100,  
            show=False,
            wait_time=0.1
