...
def parse_args():
    ...
    parser.add_argument('--source', type=str, default='GZFY', help='source domain for training')
    parser.add_argument('--target', type=str, default='XMFY', help='target domain for testing')
...
def main():
    ...
    cfg.data_root = '/public/Detection-DG/code/cerebellum-DG/'

    train_data_file = cfg.data_root + f'{source_domain}/train_annotations.json'
    test_data_file = cfg.data_root + f'{target_domain}/test_annotations.json'
    ...
    cfg.train_dataloader = dict(
        ...
        dataset=dict(
            ...
            data_prefix=dict(img=cfg.data_root + f'/{source_domain}/'),
    ...
    cfg.test_dataloader = dict(
        ...
        dataset=dict(
            ...
            data_prefix=dict(img=cfg.data_root + f'/{target_domain}/'),
            test_mode=True,
    ...
    # 其他训练参数
    cfg.work_dir = f'./work_dirs/cerebellum_detection_faster_rcnn_{source_domain}To{target_domain}{args.suffix}'  # 更新工作目录名称
    cfg.optim_wrapper.optimizer.lr = 0.02 / 16
    cfg.train_cfg.max_epochs = 36
    cfg.train_cfg.val_interval = 1
    
    # 使用 Faster R-CNN 预训练模型
    cfg.load_from = 'checkpoints/faster_rcnn_r101_fpn_1x_coco_20200130-f513f705.pth'
    
    cfg.test_evaluator = dict(
        type='CocoMetric',
        ann_file=osp.join(cfg.data_root, f'{target_domain}/coco_annotations.json'),
        metric='bbox',
        format_only=False,
        classwise=True,
        prefix='test'
    ...
    cfg.default_hooks.update({
        'logger': dict(
            type='LoggerHook',
            interval=100,
            log_metric_by_epoch=True,  # 确保按epoch记录指标
        )
    })

    # 简化visualizer配置，只保留wandb后端
    cfg.visualizer = dict(
        ...
        vis_backends=[
            dict(
                ...
                init_kwargs={
                    'project': 'cerebellum_detection_singleDG',
                    'entity': 'nekok-hkust',
